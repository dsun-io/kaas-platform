...
### Orchestrator (编排器) 开发模式
1. **定义路由层**：在 `backend/` 中通过 FastAPI 定义统一的入口（如 `POST /v2/chat`）。
2. **状态管理**：实现基于 Redis 或内存的会话状态管理，用于追踪 CoR 进度。
3. **插件化编排**：设计插件系统，动态加载不同的 CoR 角色或 AI 工具。

### CoR (角色链) 定义模式
1. **角色契约**：定义每个角色的 `InputSchema` 和 `OutputSchema`（Pydantic 模型）。
2. **逻辑封装**：角色内部可调用 AI (FastGPT/Dify) 或执行硬编码业务逻辑（如 ERP 查询）。
3. **状态流转**：定义角色如何将处理结果传递给下一个角色，或返回给编排器。

### 前后端集成模式
1. **API 契约**：使用 OpenAPI (Swagger) 自动生成的文档作为前后端对接唯一标准。
2. **CORS 配置**：后端需正确配置跨域策略，允许前端访问。
3. **监控看板**：前端实现对 Orchestrator 内部 CoR 流转状态的可视化监控。
