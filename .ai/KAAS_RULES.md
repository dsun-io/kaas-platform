...
**项目名称**：KaaS（Knowledge as a Service）—— 为传统制造业提供 AI 岗位能力托管。

**当前业务重点**：丝网行业 AI 智能客服 (SaaS)，基于 CoR (Chain of Role) 架构实现复杂电商咨询的自动化处理。

### 当前阶段
**v2 架构重构期** —— 核心目标是构建高性能 Orchestrator (编排器) 与灵活的 CoR 逻辑链，实现前后端分离。

### 技术栈 (v2.0)
- **AI 编排**：自定义 Orchestrator + CoR 角色链逻辑
- **AI 引擎**：FastGPT / Dify API 集成
- **后端**：Python 3.12+ + FastAPI (位于 `backend/`)
- **前端**：现代 Web 框架 (位于 `frontend/`)
- **数据库**：PostgreSQL (业务数据) + Redis (缓存/状态)
- **开发环境**：Windows 本地 + Docker (未来部署)
...
- **项目结构 (v2)**：
  ...
  ├── backend/                 # 后端服务（FastAPI, Orchestrator, CoR）
  ├── frontend/                # 前端管理后台
  ├── .qwen/                   # AI CLI 规则与配置（本项目的大脑）
  └── README.md                # 项目总纲
  ```
...
### 电商客服通用模式
- **核心流程**：咨询引导 → 规格确认 → 报价/计算 → 议价/促成 → 订单追踪。
- **痛点**：非标产品规格多、计算复杂、业务员回复慢、专业知识依赖度高。
- **v2 目标**：通过 CoR 将上述流程标准化，由不同 Role 分工处理。

### 行业案例：安平丝网
- **背景**：中国丝网之乡，全国 70%+ 产量。
- **产品逻辑**：多为非标定制（如牛栏网、勾花网），报价依赖于丝径、孔距、高度、卷长等多个变量的实时计算。
- **目标客户**：日均 100-300 次咨询的中等规模商家。
...
2. **时间倒序排列（强制）**：
   - 最新的一次执行记录必须写在**最上面**。
   - **绝对禁止**追加到日志末尾。
   - **写入方式**：匹配该 Tab 标题作为锚点（如 `🔧 **Runner 执行日志**`），将"新日志 + 换行 + 旧日志完整内容"一起作为 `new_str` 更新。
...
## 这是一个什么项目？
KaaS（Knowledge as a Service），为传统制造业提供 AI 岗位能力托管，首先落地丝网行业的 AI 智能客服系统。

## 核心技术栈？
v2 架构采用 Orchestrator (编排器) + CoR (角色链) + FastGPT/Dify 集成，后端使用 FastAPI，实现前后端分离。

## 当前目标？
正在进行 v2 架构重构，搭建核心编排器与前后端基础框架。

## 护城河
我们不只是通用的 AI 聊天工具，我们深入制造业垂直场景，理解复杂的非标报价逻辑、行业术语以及产业带生态，并将其转化为可托管的 AI 能力。
