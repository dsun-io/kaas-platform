...
function QuotationRow({ q, onSelect }: { q: Quotation; onSelect: (q: Quotation) => void }) {
  const statusBadge = STATUS_BADGE[q.status] ?? { variant: 'secondary' as const, label: q.status };
  return (
    <TableRow className={q.unit_price === null ? 'opacity-50' : undefined}>
      <TableCell className="font-mono text-xs">{q.quotation_id}</TableCell>
      <TableCell className="text-xs">{q.customer_id}</TableCell>
