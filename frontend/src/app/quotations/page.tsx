...
import { useQuotations, type QuotationFilters, type Quotation } from './hooks/use-quotations';
    ...
    <div className="space-y-6">
      ...
      <Card>
        <CardHeader className="pb-3">
          <CardTitle>
            报价列表
            {data && <span className="ml-2 text-sm font-normal text-muted-foreground">共 {data.total} 条</span>}
          </CardTitle>
        </CardHeader>
        <CardContent>
          <QuotationTable
      ...
      <QuotationDetailSheet quotation={selectedQuotation} onClose={() => setSelectedId(null)} />
    </div>
  );
}
