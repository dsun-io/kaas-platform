...
export function CapabilityForm({ customerId, capability, onSaved }: Props) {
  ...
  const syncing = !!(syncJobId && syncJob?.status !== 'done' && syncJob?.status !== 'failed');
      ...
      <CardHeader className="pb-2">
        <div className="flex items-center justify-between">
          <CardTitle className="text-sm">
            ...
            <Badge variant={capability.is_active ? 'default' : 'secondary'} className="ml-2">
          ...
          {!editing ? (
            ...
            <div className="flex gap-1">
              <Button variant="ghost" size="sm" onClick={handleCancel} disabled={syncing}>
                取消
              </Button>
              <Button
                variant="default"
