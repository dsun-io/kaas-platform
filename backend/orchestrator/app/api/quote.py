...
async def create_quote(request: Request, db: AsyncSession = Depends(get_db_session)):
    ...
    try:
        ...
        raise HTTPException(status_code=422, detail=f"validation_error: {e}")
    ...
    if not product_spec:
        raise HTTPException(
            status_code=400,
            detail="spec_required: product_spec or raw_text is required for quote",
        )
    ...
    try:
        ...
        raise HTTPException(
            status_code=500, detail="pricing_failed: Internal pricing error"
        )
